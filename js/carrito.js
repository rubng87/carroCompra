...
function miEleccion(nombreFruta, precio, unidad) {
  // Con esta función obtenemos el nombre de la fruta, precio y la unidad de las frutas seleccionadas, Que se la hemos asignado manualmente en el HTML
  ...
  carrito.innerHTML += `<p><i class="fas fa-trash-alt fa-sm" style="color:red" onclick="borrarProducto(${
    listaFrutas.length - 1
  })"></i> ${nombreFruta} ${cantidad} ${unidad} x ${precio} €/${unidad} = ${precioTotal} €</p>`; // le añadimos una "p" y le asignamos todo lo que queremos que aparezca en la lista que se va acumulando de compra como, "nombreFruta, cantidad, precio, unidad y al final el resultado."
